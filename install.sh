...
openvpn_setup(){ [[ -f "$VPN" ]] || { log 'OpenVPN server is not configured. Launching the official Nyr installer.'; install -d -m700 /root/mehrvpn-bootstrap; curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/Nyr/openvpn-install/master/openvpn-install.sh -o /root/mehrvpn-bootstrap/openvpn-install.sh; chmod 700 /root/mehrvpn-bootstrap/openvpn-install.sh; bash /root/mehrvpn-bootstrap/openvpn-install.sh; }; [[ -f "$VPN" && -x /etc/openvpn/server/easy-rsa/easyrsa && -f /etc/openvpn/server/client-common.txt ]] || die 'OpenVPN setup is incomplete.'; systemctl is-active --quiet openvpn-server@server.service || die 'OpenVPN service is not healthy.'; }
configure(){ local host=$1 port=$2 origin san; origin="https://$host:$port"; [[ $port == 443 ]] && origin="https://$host"; printf 'MEHRVPN_DB=%s\nMEHRVPN_PUBLIC_URL=%s\n' "$DB/panel.db" "$origin" > "$ENV"; chmod 600 "$ENV"; [[ $host =~ ^[0-9]+(\.[0-9]+){3}$ ]] && san="IP:$host" || san="DNS:$host"; openssl req -x509 -newkey rsa:3072 -sha256 -nodes -days 365 -keyout "$ETC/tls/panel.key" -out "$ETC/tls/panel.crt" -subj "/CN=$host" -addext "subjectAltName=$san" >/dev/null 2>&1; chmod 600 "$ETC/tls/panel.key"; cat >/etc/nginx/conf.d/mehrvpn.conf <<EOF
...
wait_agent(){ for i in {1..30}; do [[ -S /run/mehrvpn/control.sock ]] && return; sleep 1; done; die 'Agent socket did not appear.'; }
